// player.js - 集成Waline评论系统的完整实现

const selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '[]');
const customAPIs = JSON.parse(localStorage.getItem('customAPIs') || '[]');
let walineInstance = null; // 存储Waline实例
...
let commentsVisible = false; // 评论是否可见
...
function setupComments() {
  ...
  if (toggleBtn) {
    ...
    // 默认隐藏评论
    document.getElementById('waline-container').style.display = 'none';
    document.getElementById('toggleComments').innerHTML = '<span>显示评论</span>';
  }
  
  // 确保密码验证通过后才加载评论
  if (isPasswordVerified()) {
    setTimeout(initWaline, 3000); // 延迟3秒加载
  }
}
...
function initWaline() {
  ...
  // 如果没有ID则使用当前URL作为标识
  const path = videoId || window.location.pathname + window.location.search;
  ...
  // 初始化Waline
  walineInstance = Waline.init(walineConfig);
  
  // 如果评论已设置为可见，显示评论容器
  if (commentsVisible) {
    document.getElementById('waline-container').style.display = 'block';
  }
}
...
function toggleCommentsVisibility() {
  ...
  if (container.style.display === 'none') {
    ...
    toggleBtn.innerHTML = '<span>隐藏评论</span>';
    ...
    toggleBtn.innerHTML = '<span>显示评论</span>';
...
function handleKeyboardShortcuts(e) {
  ...
  if (e.key === 'c' || e.key === 'C') {
    ...
    showShortcutHint(commentsVisible ? '隐藏评论' : '显示评论', 'comment');
    e.preventDefault();
  }
}
...
function showShortcutHint(text, direction) {
  ...
  } else if (direction === 'comment') {
    iconElement.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>';
  }
...
function renderEpisodes() {
  ...
  episodes.forEach((episode, index) => {
    ...
    html += `
      <button id="episode-${realIndex}" onclick="playEpisode(${realIndex})" 
  ...
  try {
    ...
    if (existingIndex !== -1) {
      ...
      if (videoInfo.episodes && videoInfo.episodes.length > 0) {
        if (!existingItem.episodes || !Array.isArray(existingItem.episodes) || 
